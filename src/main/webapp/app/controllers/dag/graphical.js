...
export default MultiTableController.extend({
  ...
  columns: ColumnDefinition.make([{
    ...
    headerTitle: 'ContainerGroup Name',
    contentPath: 'containerGroupName'
  },{
    id: 'status',
    headerTitle: 'Status',
  ...
  viewData: Ember.computed("model", function () {
    var model = this.get("model"),
        dag, vertices, daemonVertices, entities;
    ...
    daemonVertices = this.get('model.firstObject.dag.daemonVertices') || [];
    entities = {};
    ...
    daemonVertices.forEach(function (daemonVertex) {
      daemonVertex.data = entities[daemonVertex.vertexName];
    });

    return {
      vertices: vertices,
      edges: dag.get('edges'),
      daemonVertices: daemonVertices,
