...
export default Ember.Component.extend({
  ...
  normalizedTabs: Ember.computed("tabs", "application.currentPath", function () {
    ...
    return tabs.map(function (tab) {
      var active = tab.routeName === activeRouteName;

      if(that.compareOutmostRoutes) {
        var outmostActiveRouteName = activeRouteName.split(".")[0];
        active = tab.routeName.indexOf(outmostActiveRouteName) === 0;
      }
