...
export default function downloadDagZip(dag, options) {
  var opts = options || {},
      ...
      itemsToDownload = [
        {
          url: getUrl('MR3_APPLICATION', dag.get("appID")),
          context: { name: 'application', type: 'MR3_APPLICATION' },
          onItemFetched: processSingleItem
        },
        {
          url: getUrl('MR3_DAG', dagID),
          context: { name: 'dag', type: 'MR3_DAG' },
          onItemFetched: processSingleItem
        },
        {
          url: getUrl('MR3_VERTEX', dagID),
          context: { name: 'vertices', type: 'MR3_VERTEX', part: 0 },
          onItemFetched: processMultipleItems
        },
        {
          url: getUrl('MR3_TASK', dagID),
          context: { name: 'tasks', type: 'MR3_TASK', part: 0 },
          onItemFetched: processMultipleItems
        },
        {
          url: getUrl('MR3_TASK_ATTEMPT', dagID),
          context: { name: 'task_attempts', type: 'MR3_TASK_ATTEMPT', part: 0 },
          onItemFetched: processMultipleItems
        }
      ],
  ...
  function getUrl(type, dagID, fromID) {
    ...
    if (type === 'MR3_DAG' || type === 'MR3_APPLICATION') {
      url = `${baseurl}/${type}/${dagID}`;
    } else {
      url = `${baseurl}/${type}?primaryFilter=MR3_DAG:${dagID}&limit=${queryBatchSize}`;
      if (!!fromID) {
