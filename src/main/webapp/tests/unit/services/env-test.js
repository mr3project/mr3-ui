...
test('setComputedENVs test', function(assert) {
  ...
  assert.equal(service.ENV.APP.contactAm, false);
});

test('setComputedENVs test with ENV.contactAm set to true', function(assert) {
  let service = this.subject();
  window.ENV = {
		contactAm: true,
  };
  service.collateConfigs();
  assert.equal(service.ENV.APP.contactAm, true);
});

test('setComputedENVs test with ENV.contactAm set to false', function(assert) {
  let service = this.subject();
...
test('Validate config/default-app-conf.js', function(assert) {
  ...
  assert.equal(service.get("app.paths.timeline.dag"), "MR3_DAG");
});
