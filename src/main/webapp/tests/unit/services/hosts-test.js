...
test('Test correctProtocol', function(assert) {
  ...
  // Correction
  assert.equal(service.correctProtocol(""), "http://");
...
test('Test host URLs', function(assert) {
  ...
  //assert.equal(service.get("am"), "http://");
  assert.equal(service.get("am"), "http://am");
});
...
test('Test host URLs with ENV set', function(assert) {
  ...
  window.ENV = {
    hosts: {
      ...
      am: "https://localhost:4444"
    }
  };
  assert.equal(service.get("timeline"), "http://localhost:3333");
  assert.equal(service.get("am"), "http://localhost:4444");
});
