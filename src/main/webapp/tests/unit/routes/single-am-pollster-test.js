...
test('canPoll test', function(assert) {
  ...
  route.set("loadedValue.isComplete", true);
  assert.notOk(route.get("canPoll"));

  route.set("loadedValue.isComplete", undefined);
  assert.notOk(route.get("canPoll"));
});
...
test('_loadedValueObserver test', function(assert) {
  ...
  assert.equal(route.get("polledRecords.0"), null);
...
test('_loadedValueObserver test - With contactAm true', function(assert) {
  let route = this.subject({
    polling: {
      resetPoll: function () {}
    },
...
test('_loadedValueObserver test - With contactAm false', function(assert) {
  let route = this.subject({
    polling: {
      resetPoll: function () {}
    },
